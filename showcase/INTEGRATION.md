...
- Copy `showcase/product-dev-os.html` into your site repo.
- Put it in the **same place as your other pages** (e.g. next to `index.html` and `risk.html`), so the URL is:
  - **`https://mikelyngaas.github.io/pages/product-dev-os.html`**

If your site uses a build step (e.g. Jekyll), add `product-dev-os.html` so it’s included in the build output at the root (or adjust the path in the snippet below).
...
```html
<a href="https://mikelyngaas.github.io/pages/product-dev-os.html">Product Dev OS</a>
...
(Adjust the link if you put `product-dev-os.html` in a different path, e.g. `href="/pages/product-dev-os.html"` or `href="/projects/product-dev-os.html"`.)
...
```markdown
[Product Dev OS](https://mikelyngaas.github.io/pages/product-dev-os.html)
...
Open `product-dev-os.html` and replace the placeholder sections:
...
The “← Back” link on the Product Dev OS page points to `https://mikelyngaas.github.io/`. If your site’s root URL is different, update it in `product-dev-os.html`.
